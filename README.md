...
'auth'  => [
    ...
    'auth_group'        => 'deneptauth_group', // 用户组数据不带前缀表名
    'auth_group_access' => 'deneptauth_group_access', // 用户-用户组关系不带前缀表名
    'auth_rule'         => 'deneptauth_rule', // 权限规则不带前缀表名
    'auth_user'         => 'deneptmember', // 用户信息不带前缀表名
],
```
...
```
------------------------------
-- dbo_deneptauth_rule，规则表，
-- id:主键，name：规则唯一标识, title：规则中文名称 status 状态：为1正常，为0禁用，condition：规则表达式，为空表示存在就验证，不为空表示按照条件验证
------------------------------
 DROP TABLE IF EXISTS `dbo_deneptauth_rule`;
CREATE TABLE `dbo_deneptauth_rule` (  
...
------------------------------
-- dbo_deneptauth_group 用户组表， 
-- id：主键， title:用户组中文名称， rules：用户组拥有的规则id， 多个规则","隔开，status 状态：为1正常，为0禁用
------------------------------
 DROP TABLE IF EXISTS `dbo_deneptauth_group`;
CREATE TABLE `dbo_auth_group` ( 
    `id` mediumint(8) unsigned NOT NULL AUTO_INCREMENT, 
...
------------------------------
-- dbo_deneptauth_group_access 用户组明细表
-- uid:用户id，group_id：用户组id
------------------------------
DROP TABLE IF EXISTS `dbo_deneptauth_group_access`;
CREATE TABLE `dbo_auth_group_access` (  
...
- 规则表（dbo_deneptauth_rule） 
- 用户组表(dbo_deneptauth_group) 
- 用户组明显表（dbo_deneptauth_group_access）

我们在规则表中定义权限规则， 在用户组表中定义每个用户组有哪些权限规则，在用户组明显表中定义用户所属的用户组。 

下面举例说明：

我们要判断用户是否有显示一个操作按钮的权限， 首先定义一个规则， 在规则表中添加一个名为 show_button 的规则。 然后在用户组表添加一个用户组，定义这个用户组有show_button 的权限规则（dbo_deneptauth_group表中rules字段存得时规则ID，多个以逗号隔开）， 然后在用户组明细表定义 UID 为1 的用户 属于刚才这个的这个用户组。 

## 使用
判断权限方法
```
// 引入类库
use denept\auth\Auth2;
